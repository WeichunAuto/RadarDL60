    ...
    def start_training(self):
        ...
        for epoch in tqdm(range(self.epochs)):
            ...
            if v_loss < best_v_loss:
                best_v_loss = v_loss
                best_v_epoch = epoch
                torch.save(self.model.state_dict(), "harhn_best_v_model_"  + self.formatted_time + "_" + str(self.dropout) + "_.tar")  # 保存训练后的模型
            if t_loss < best_t_loss:
                best_t_loss = t_loss
                best_t_epoch = epoch
                torch.save(self.model.state_dict(), "harhn_best_t_model_"  + self.formatted_time + "_" + str(self.dropout) + "_.tar")  # 保存训练后的模型
...
dropout = 0.
et = ExecuteTrainHARHN(epochs=30, dropout=dropout)
t_loss, v_loss, _ = et.start_training()
et.visualize_loss(t_loss, v_loss)
