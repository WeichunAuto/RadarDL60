...
class ExecuteTrainBiLSTM:
    def __init__(self, window_size=5, fs=2000, is_shuffle=False, epochs=500):
        ...
        self.train_rate = 0.8
        self.y_test = None
        self.train_loader, self.test_loader = PrepareTrainData(is_shuffle=is_shuffle).train_dataloader(is_complex=True, is_normalize=True)
    ...
    def initialize_model(self):
        lr = 0.001
        loss_fun = nn.MSELoss()
        model = RadarBiLSTM(n_features=200)
    ...
    def start_training(self):
        ...
        for epoch in tqdm(range(self.epochs)):
            ...
            if v_loss < best_v_loss:
                best_v_loss = v_loss
                torch.save(self.model.state_dict(), "bi_lstm_best_v_model_" + self.formatted_time + ".tar")  # 保存训练后的模型
                print(f"v saved.")
            if t_loss < best_t_loss:
                best_t_loss = t_loss
                torch.save(self.model.state_dict(), "bi_lstm_best_t_model_" + self.formatted_time + ".tar")  # 保存训练后的模型
                print("t saved.")
...
et = ExecuteTrainBiLSTM(epochs=20)
